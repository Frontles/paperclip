...
export default function PlinkoGameScreen() {
  ...
  const { timerLabel, isExtraTime, extraMinute, matchMinute, phase } = useMatchTimer({
    isRunning,
  ...
  const isPaused = phase === 'half_time' || phase === 'extra_intro' || phase === 'extra_half_time';
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.canvas} onLayout={handleLayout}>
        ...
        {canvasSize.width > 0 && <DropZoneArc canvasWidth={canvasSize.width} />}
        ...
        {balls.map((ball, idx) => (
          <Ball key={idx} x={ball.x} y={ball.y} color={ball.color} badge={ball.badge} />
        ))}
